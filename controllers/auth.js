...
exports.sendMail= asyncHandler(async(req,res,next) =>{
    ...
    const URL=`${req.protocol}://${req.get('host')}/api/v1/auth/verify/${verifyToken}`
    const message='Please click on verify to verify your account and start uploading photo'
    try {
        await sendEmail({
            email:user.email,
...
//access        public
exports.verifyMail= asyncHandler(async(req,res,next) =>{
    ...
    if(user.VerifyMailTokenExpire<Date.now()){
        return next(new ErrorResponse('token expires please generate new token to verify',404))
    }
